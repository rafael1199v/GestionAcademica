...
function VacancyUpdateForm() {
  ...
  const getSubjectsWithCareers = async (vacancyToUpdate) => {
    try {
      //A diferencia de la creacion, ahora se necesita seleccionar la carrera
      const newSubjects = await vacancyService.getSubjectsWithCareers();
      const newCareers = newSubjects.filter(
        (subject) => subject.id == vacancyToUpdate.subjectId
      )[0].careers;
  ...
  const getVacancyToUpdate = async () => {
    try {
      //Los id me vienen como numeros
      const vacancyToUpdate = await vacancyService.getVacancyToUpdate(id);
      vacancyToUpdate.careerId = vacancyToUpdate.careerId.toString();
      vacancyToUpdate.subjectId = vacancyToUpdate.subjectId.toString();

      await getSubjectsWithCareers(vacancyToUpdate);
      setVacancy(vacancyToUpdate);
    } catch (error) {
  ...
  const handleChangeSubject = (subjectId) => {
    ...
    const newFilteredCareers = subjects.filter(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess("Vacante actualizada con exito");
      setTimeout(() => {
        setSuccess(null);
      }, 3000);
    } catch (error) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-dvh">
      <h1 className="text-2xl">Actualizar Vacante</h1>
      <form
        className="w-lg mx-auto mt-10 bg-white rounded-lg p-8 dark:bg-gray-900"
      ...
      >
        ...
        <div className="mb-5">
          ...
          <input
            ...
            required
            onChange={(e) =>
              setVacancy({ ...vacancy, startTime: e.target.value })
            }
            value={vacancy.startTime}
          />

          {errors && <p className="text-red-500"> {errors.startTime}</p>}
        </div>
        <div className="mb-5">
          <label
            htmlFor="endTime"
          ...
          <input
            ...
            required
            onChange={(e) =>
              setVacancy({ ...vacancy, endTime: e.target.value })
            }
            value={vacancy.endTime}
          />

          {errors && <p className="text-red-500"> {errors.endTime}</p>}
        </div>
        <div className="mb-5">
          <label
            htmlFor="subject"
          ...
          {errors && <p className="text-red-500"> {errors.subjectId}</p>}
        </div>
        ...
        <div className="mb-5">
          ...
          <select
            ...
            required
            onChange={(e) =>
              setVacancy({ ...vacancy, careerId: e.target.value })
            }
          ...
          {errors && <p className="text-red-500"> {errors.careerId}</p>}
        </div>
